...
const ContactForm = () => {
  ...
  const sendEmail = (data, e) => {
    e.preventDefault();
    window.emailjs
      .send("service_p28c6ys", "template_pezzinj", data)
      .then((res) => {
        e.target.reset();
    ...
    <>
      {!toggleSuccess ? (
        ...
        <>
          <Modal
            message={
              "El mensaje fue enviado con exito! Nos comunicaremos lo más pronto posible"
            }
            textButton={"Aceptar"}
            title={"Email enviado"}
            iconURL={check_icon}
      ...
      {!toggleFail ? (
        ...
        <>
          <Modal
            message={
              "No fue posible enviar el correo, por favor intente nuevamente."
            }
            textButton={"Aceptar"}
